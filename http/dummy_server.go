...
func (h *UploadHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	files, err := ioutil.ReadDir(h.UploadDir)
	if err != nil {
		http.Error(w, "Unable to read upload dir", http.StatusInternalServerError)
		return
	}
	for _, UploadedFile := range files {
		if UploadedFile.Name() == header.Filename {
			header.Filename = header.Filename + "1"
		}
	}
	defer file.Close()
